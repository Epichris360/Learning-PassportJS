  ...
  passport.use(new FacebookStrategy({
    ...
    function(req, token, refreshToken, profile, done) {
        ...
        process.nextTick(function() {
            ...
            if (!req.user) {
              // check if user in the database based on their facebook id
              User.findOne({'facebook.id': profile.id}, function(err, user){
                ...
                if(user){
                  ...
                  if(!user.facebook.token){
                    ...
                    user.facebook.email = profitable.emails[0].value;
      ...
      process.nextTick(function(){
        if (!req.user) {
          User.findOne({'twitter.id': profile.id}, function(err, user){
            ...
            // if the user is found then log them in
            if(user){
              //if user exists and token is undefined(because the user at somepoint already connected account and then disconected it)
